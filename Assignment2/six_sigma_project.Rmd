...
###### Data for defective phones according to touch screen supplier.
...
In fact there is a significant difference between the delivery time of both companies. If possible, commissioning the first should be preferred. Since we had time on our hands, PhoneNow agreed to collect data for one more month after this test, only using the first shipping company in the test from above. **Does it make sense to do this?? Have a look please. If not, the code below for the chart and Six-Sigma measurements can be easily changed. **
...
# ALex: I did not really understand this, so used the simle suggestion. 
```
