...
The managers defined the customer satisfaction as acceptable when it is distributed as $N(80, 15)$.
...
faults<-sum((satisfaction<LSL))
faults/N
```

For satisfaction, obviously, only a lower bound LSL can be defined, because exceeding the threshold is not a bad thing. It is estimated as `r LSL`.
Thereby, a proportion of `r round(faults/N, 2)` is observed. Per million parts this gives `r faults/N*10^6`, which means the process is out of control.
...
```{r, results="hide"}
x <- seq(min(satisfaction), max(satisfaction), length = 100)
f1 <- dnorm(x, mean = mean(satisfaction), sd = sqrt(var(satisfaction)))
f2 <- dnorm(x, mean = mu_opt, sd = sigma_opt)
...
```
The dotted vertical lines shows the threshold under which a customer satisfaction is considered as a fault. In fact only four faults are observed.
The dotted gaussian is the observed process and the steeper, black gaussian is the optimal process which should be achieved. We can see that both the mean is clearly too low and the variance is slightly too large.
...
LSL <- (-6) * sigma_opt + mu_opt
N=500
satisfaction = rnorm(N,80-rnorm(1,-1,1),sqrt(10)+runif(1,0,2))
satisfaction[satisfaction>100]=runif(1,90,100)
faults<-sum((satisfaction<LSL))
faults/N
```
