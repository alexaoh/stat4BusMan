...
We want to check if the process is a Six-Sigma process. The process is assumed to be optimal if the total time until the customer receives the product is distributed as $N(25, 1)$. 
...
Thereby, a proportion of `r round(faultsj/Nj, 3)` is observed. Per million parts this gives `r faultsj/Nj*10^6`, which means the process is out of control.
In fact it is a 4-sigma process.
...
As earlier, we want to check if the process is Six-Sigma and if it is out-of-control using the Six-Sigma criterion. The process is still assumed to be optimal if the total time until the customer receives the product is distributed as $N(25, 1)$. 
...
```{r, results = "hide"}
qcc(total_time_final, type = "xbar.one", center = 25, std.dev = 1, nsigma = 6)
...
The observed process does not perfectly match the optimal one, but is very close. The variance is still slightly bigger, the mean is in fact higher than it was defined by managers in the beginning.
